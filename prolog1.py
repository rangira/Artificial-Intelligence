...
def unify (srcTerm, srcEnv, destTerm, destEnv) :
    ...
    #print "srcTerm srcEnv destTerm destEnv srcTerm.args srcTerm.pred destTerm.pred %s %s %s %s %s %s %s" % (srcTerm , srcEnv, destTerm, destEnv,srcTerm.args,srcTerm.pred,destTerm.pred)
...
def search (term) :
    ...
    while stack :
        ...
        #print "c.rule.goals %s" % c.rule.goals
        if c.inx >= len(c.rule.goals) :       # Is this one finished?
            if c.parent == None :             # Yes. Our original goal?
                ...
                else     : print "Yes there is a solution"        # have a solution
                continue
        ...
        for rule in rules :                     # Walk down the rule database
            ...
            #print " child.env before  %s" %  child.env
            ans = unify (term, c.env, rule.head, child.env)
            #print " child.env after %s" %  child.env
